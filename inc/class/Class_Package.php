...
class Package extends RsGlobal
{
	...
	public $id = 0, $name = '',$price = 0, $place_from = '',$status="", $resort_to = '', $days = '', $type = '', $details = '', $total_seat = '', $date = '', $last_updated= '';
	...
	public function addPackage($data)
	{
		...
		$sql = "INSERT INTO $this->tableName (`name`,`place_from`, `resort_to`, `days`, `type`, `details`, `total_seat`, `date`, `price`,`status`) VALUES('".$data['name']."','".$data['place_from']."','".$data['resort_to']."','".$data['days']."','".$data['type']."','".$data['details']."','".$data['total_seat']."','".$formatedDate."','".$data['price']."','".$data['status']."')";
	...
	public function update($id,$data)
	{
		$date=date_create($data['date']);
		$formatedDate = date_format($date,"Y-m-d");
		$sql = "UPDATE $this->tableName SET name='".$data['name']."', place_from='".$data['place_from']."', resort_to='".$data['resort_to']."', days='".$data['days']."', type='".$data['type']."', details='".$data['details']."',  total_seat='".$data['total_seat']."', date='".$formatedDate."', price='".$data['price']."', status='".$data['status']."' WHERE id='".$id."'";
		$this->db->con->query($sql);
	}
